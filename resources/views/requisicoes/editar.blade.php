            ...
            <div class="card text-white bg-danger mb-3" id="msg" style="padding: 5px; display: none;">
			  ...
			  @if(!empty($ignore))
                ...
                @endif
             	...
             	<div class="form-group row">
                    	...
                    	<div class="col-sm-2">
                     	 		<input type="datetime-local" name="date" id="dat" class="form-control"  value="{{ str_replace(' ','T',$r->dat) }}">
                    	</div>


                        <label for="fim" class="col-sm-1 col-form-label">Fim</label>
                    	<div class="col-sm-2">
                     	 		<input type="datetime-local" name="fim" id="fim" class="form-control"  value="{{ str_replace(' ','T',$r->fim) }}">
                    	</div>
                            ...
                            <div class="col-sm-2">
                              <input type="number" name="horas" id="horas" class="form-control"  value="{{$r->horas}}">
                            </div>


                    		<label for="num_ordem" class="col-sm-1 col-form-label">Nº Ordem</label>
                            <div class="col-sm-2">
                              <input type="number" name="num_ordem" id="num_ordem" class="form-control"  value="{{$r->num_ordem}}">
                            </div>
			 ...
			 <div class="form-group row">
                     ...
                     <label for="cod_fun1" class="col-sm-1 col-form-label">Funcionario 1</label>
                        <div class="col-sm-2">
                           <select class="form-control selectpicker" data-live-search="true"  name="cod_fun1"  placeholder="Funcionario" required>
                                            <option value="">Selecione</option>

                                        @if($funcionarios)
                                            @foreach($funcionarios as $f)
                                                <option value="{{ $f->num_matricula}}" @if(trim($r->cod_fun1)==$f->num_matricula) selected="selected" @endif>{{ $f->num_matricula }} - {{ $f->nom_funcionario }}</option>
                                            @endforeach
                                        @endif
                            </select>
                        </div>
                        ...
                        <div class="col-sm-2">
                           <select class="form-control selectpicker" data-live-search="true"  name="cod_fun2"  placeholder="Funcionario">
                                        ...
                                        @if($funcionarios)
                                            @foreach($funcionarios as $f)
                                                <option value="{{ $f->num_matricula}}" @if(trim($r->cod_fun2)==$f->num_matricula) selected="selected" @endif>{{ $f->num_matricula }} - {{ $f->nom_funcionario }}</option>
                                            @endforeach
                                        @endif
                            </select>
                        </div>
                    ...
                    <div class="col-sm-1">
                      <input type="number" name="solicitadas" id="solicitadas" class="form-control" onchange="soma_mistura()"  required value="{{$r->solicitada}}" readonly="readonly">
                    </div>
                    ...
                    <div class="col-sm-1">
                      <input type="number" name="misturas" id="misturas" class="form-control"  onchange="soma_mistura()" required value="{{$r->misturas}}" readonly="readonly">
                    </div>


                     <label for="total_misturas" class="col-sm-1 col-form-label">Total</label>
                    <div class="col-sm-1">
                      <input type="text" name="total_misturas" id="total_misturas" class="form-control"  required readonly="readonly" >
                    </div>
