...
<main>
    <div class="container-fluid">
        ...
        <div class="card mb-4">
         ...
         -->
       <form name="myForm"  onsubmit="return validateForm()"  role="form" action="{{ url('requisicoes/novo')}}" class="form" method="post" enctype="multipart/form-data">
        <div class="card mb-4">
            ...
            <div class="card text-white bg-danger mb-3" id="msg" style="padding: 5px; display: none;">
			 ...
			 <input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
			  ...
			  <div class="form-group">
                    <div class="form-check">
                      <input class="form-check-input" type="checkbox" value="S" id="mistura" name="mistura"  checked>
				 ...
				 <div class="form-group row">
                    ...
                    <div class="col-sm-2">
                        <input type="text" name="cod_item" id="cod_item"  class="form-control"  value="{{old('cod_item')}}" onfocusout="getitem('{{ url('getitem')}}',this.value)">
             	...
             	<div class="form-group row">
                    	...
                    	<div class="col-sm-2">
                     	 		<input type="datetime-local" name="date" id="dat" class="form-control"  value="<?php echo date('Y-m-d'); ?>">
                    	</div>


                        <label for=fim" class="col-sm-1 col-form-label">Fim</label>
                    	<div class="col-sm-2">
                     	 		<input type="datetime-local" name="fim" id="fim" class="form-control"  value="<?php echo date('Y-m-d'); ?>">
                    	</div>
                    ...
                    <div class="col-sm-2">
                      <input type="number" name="num_ordem" id="num_ordem" class="form-control"  value="{{old('num_ordem')}}">
                    </div>
			 ...
			 <div class="form-group row">
                     ...
                     <label for="cod_fun1" class="col-sm-1 col-form-label">Funcionario 1</label>
                        <div class="col-sm-2">
                           <select class="form-control selectpicker" data-live-search="true"  name="cod_fun1"  placeholder="Funcionario">
                                            <option value="">Selecione</option>
                                        @if($funcionarios)
                                            @foreach($funcionarios as $f)
                                                <option value="{{ $f->num_matricula}}" @if(old('cod_fun1')==$f->num_matricula) selected @endif>{{ $f->num_matricula }} - {{ $f->nom_funcionario }}</option>
                                            @endforeach
                                        @endif
                            </select>
                        </div>
                        ...
                        <div class="col-sm-2">
                           <select class="form-control selectpicker" data-live-search="true"  name="cod_fun2"  placeholder="Funcionario">
                                            <option value="">Selecione</option>
                                        @if($funcionarios)
                                            @foreach($funcionarios as $f)
                                                <option value="{{ $f->num_matricula}}" @if(old('cod_fun2')==$f->num_matricula) selected @endif>{{ $f->num_matricula }} - {{ $f->nom_funcionario }}</option>
                                            @endforeach
                                        @endif
                            </select>
                        </div>
                    ...
                    <div class="col-sm-1">
                      <input type="number" name="solicitadas" id="solicitadas" class="form-control" onfocusout="soma_mistura()"  required value="{{old('solicitadas')}}">
                    </div>
                    ...
                    <div class="col-sm-1">
                      <input type="number" name="misturas" id="misturas" class="form-control"  onfocusout="soma_mistura()" required value="{{old('misturas')}}">
                    </div>


                     <label for="total_misturas" class="col-sm-1 col-form-label">Total</label>
                    <div class="col-sm-1">
                      <input type="text" name="total_misturas" id="total_misturas" class="form-control"  required readonly="readonly" value="{{old('total_misturas')}}">
                    </div>
